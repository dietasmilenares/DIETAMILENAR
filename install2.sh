...
read -rp "  Stripe Secret Key [Enter = pular]: " STRIPE_KEY; STRIPE_KEY=${STRIPE_KEY:-sk_test_$(openssl rand -hex 24)}
...
if [[ -f "$CHATW" ]]; then
  NEW_URL="http://${DOMAIN}/widget/index.php?room=dieta-faraonica"
  sed -i "s|https://socialproof-production.up.railway.app/widget/index.php?room=dieta-faraonica|${NEW_URL}|g" "$CHATW"
  log_status "Chat Widget configurado para: $NEW_URL"
else
  log_warn "ChatWidget.tsx nao encontrado em $CHATW"
fi
